...
while($row = $query->fetch(PDO::FETCH_ASSOC)){
    //$leng = "SELECT Cantidad, Precio FROM Historial_Orden_C O LEFT JOIN Historial_C H ON O.IDH = H.IDH WHERE Producto=:pro AND Fecha = '2021-07-01'";
    $leng = "SELECT Cantidad, Precio, propina FROM Historial_Orden_C O LEFT JOIN Historial_C H ON O.IDH = H.IDH WHERE Producto=:pro AND Fecha BETWEEN '$year-$numes-01' AND '$year-$numes-$end'";
    ...
    while($xow = $consulta->fetch(PDO::FETCH_ASSOC)){
        ...
        if($xow['propina'] == 1){
            $propi = $xow['Cantidad'] * 0.1;
            $propina = $propina + $propi;
        }
    }
    if($potal != 0){
...
$bot = "SELECT Precio FROM Historial_Orden_C WHERE Producto='descuento' AND Fecha BETWEEN '$year-$numes-01' AND '$year-$numes-$end'";
$ia = $conn->prepare($bot);
$ia->execute();
$descuento = 0;
...
if($descuento != 0){
    $neu = -1 * $descuento;
    $total_descuento = $global_total - $neu;
}
if($descuento == 0){
    $printer->text("TOTAL VENTAS DE MES: $".number_format(($global_total) , 2)."\n");
    $printer->text("TOTAL PROPINAS DE MES: $".number_format($propina , 2)."\n");
    $printer->text("TOTAL PROPINA: $".number_format($propina , 2)."\n");
} else {
    $printer->text("TOTAL VENTAS DE MES: $".number_format(($global_total) , 2)."\n");
    $printer->text("TOTAL DESCUENTO: -$".number_format($neu , 2)."\n");
    $printer->text("TOTAL VENTAS CON DESCUENTO: $".number_format($total_descuento , 2)."\n");
    $printer->text("TOTAL PROPINA: $".number_format($propina , 2)."\n");
}
