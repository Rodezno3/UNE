...
<body>
    ...
    <section class="inicio">
        ...
        $s = "SELECT * FROM Historial WHERE IDH=:idh";
        $q = $conn->prepare($s);
        ...
        <article class="factura">
            ...
            <?php 
            $pp = $r['IDH'];
            $pre = "SELECT * FROM Historial_Orden WHERE IDH=$pp";
            ...
            <p>
                ...
                <span>$<?php $subt = $total * 0.1; echo number_format($subt, 2); ?></span>
            </p>
            <?php } else { $subt = 0; } ?>
            <hr>
            <p>
                <strong><span>Total</span></strong>
                <strong><span>$<?php echo number_format(($total + $subt) , 2); ?></span></strong>
            </p>
            <hr>
        ...
        <article class="block_activo">
            ...
            <table id="tabla">
                ...
                if(!empty($_POST['search']) && !empty($_POST['mesa']) && !empty($_POST['fecha'])){
                    $search = $_POST['search'];
                    $sql = "SELECT * FROM Historial WHERE Cliente LIKE '%$search%' AND Mesa=:mesa AND Fecha=:fech";
                    $query = $conn->prepare($sql);
                    $query->bindParam(':mesa' , $_POST['mesa']);
                    $query->bindParam(':fech' , $_POST['fecha']);
                } elseif(!empty($_POST['search']) && !empty($_POST['mesa'])){
                    $search = $_POST['search'];
                    $sql = "SELECT * FROM Historial WHERE Cliente LIKE '%$search%' AND Mesa=:mesa";
                    $query = $conn->prepare($sql);
                    $query->bindParam(':mesa' , $_POST['mesa']);
                } elseif(!empty($_POST['search']) && !empty($_POST['fecha'])){
                    $search = $_POST['search'];
                    $sql = "SELECT * FROM Historial WHERE Cliente LIKE '%$search%' AND Fecha=:fech";
                    $query = $conn->prepare($sql);
                    $query->bindParam(':mesa' , $_POST['fecha']);
                } elseif(!empty($_POST['mesa']) && !empty($_POST['fecha'])){
                    $sql = "SELECT * FROM Historial WHERE Mesa=:mesa AND Fecha=:fech";
                    $query->bindParam(':mesa' , $_POST['mesa']);
                    $query->bindParam(':fech' , $_POST['fecha']);
                    $query = $conn->prepare($sql);
                } elseif(!empty($_POST['search'])){
                    $search = $_POST['search'];
                    $sql = "SELECT * FROM Historial WHERE Cliente LIKE '%$search%'";
                    $query = $conn->prepare($sql);
                } elseif(!empty($_POST['mesa'])){
                    $sql = "SELECT * FROM Historial WHERE Mesa=:mesa";
                    $query = $conn->prepare($sql);
                    $query->bindParam(':mesa' , $_POST['mesa']);
                } elseif(!empty($_POST['fecha'])){
                    $sql = "SELECT * FROM Historial WHERE Fecha=:fech";
                    $query = $conn->prepare($sql);
                    $query->bindParam(':mesa' , $_POST['fecha']);
                } else {
