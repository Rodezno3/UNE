...
<body>
    ...
    <section class="preorden">
        ...
        <div id="prepa">
        <?php 
        $can = "SELECT * FROM Descripcion_T WHERE IDAT=:idac";
        $cen = $conn->prepare($can);
        $cen->bindParam(':idac' , $_GET['menu']);
        $cen->execute();
        $tock = 0;
        while($cin = $cen->fetch(PDO::FETCH_ASSOC)){
            $dan = "SELECT Producto, Precio FROM Articulo_T WHERE IDART=:idmec";
            $den = $conn->prepare($dan);
            $den->bindParam(':idmec' , $cin['IDART']);
            $den->execute();
    ...
    <section class="menu_contenedor">
        <form action="php/fun/act_orden_tienda.php?id=<?= $id; ?>" method="post">
