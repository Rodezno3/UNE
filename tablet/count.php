    ...
    <section id="count">
        ...
        <article class="vida">
            ...
            <table>
                ...
                <?php
                if(isset($_POST['mes'])){
                    $xql = "SELECT * FROM Articulo_T";
                    $xuery = $conn->prepare($xql);
                    $xuery->execute();
                    $total_mensual = 0;
                    while($xow = $xuery->fetch(PDO::FETCH_ASSOC)){
                        $fan = "SELECT Cantidad, Precio FROM Historial_Orden_T O LEFT JOIN Historial_T H ON O.IDH = H.IDH WHERE Producto=:pro AND Fecha BETWEEN '2021-$numes-01' AND '2021-$numes-$end'";
