...
while($colum = $consulta->fetch(PDO::FETCH_ASSOC)){
    $s = "SELECT * FROM Menu_C WHERE IDMEC=:idme";
    $q = $conn->prepare($s);
    ...
    $printer->text( $colum['Cantidad'] . " Unidad   $". number_format(Precio($r['IDMEC']) , 2) ." $".number_format((Precio($r['IDMEC']) * $colum['Cantidad']) , 2)."   \n");
...
if(!empty($row['IDCF'])){
    ...
    $xql = "UPDATE Cliente_Frecuente SET TotalVisita=:vis , Ultima=:last , Total=:total WHERE IDCF=:id";
    $xuery = $conn->prepare($xql);
    $xuery->bindParam(':vis' , $vis);
    $xuery->bindParam(':last' , $fecha);
    $xuery->bindParam(':total' , $compra);
    $xuery->bindParam(':id' , $row['IDCF']);
    $xuery->execute();
...
if($row['propina'] == 1){
    $printer->text("SUB TOTAL: $".number_format($total , 2)."\n");
    $prop = ($total * 0.1);
    $printer->text("Propina: $".number_format($prop , 2)."\n");
    $total = $total + $prop;
}
...
$f = $_GET['id'];
if($ins == 1){
    header("location: ../php/fun/finalizar_orden.php?id=$f&deal=$deal");
} else {
