...
if(isset($_POST) && isset($_COOKIE)){
    if(!empty($_POST['name']) && !empty($_POST['mesa']) && !empty($_POST['cliente'])){
        ...
        $propina = 0;
        $sql = "INSERT INTO Activo_C (Mesa , Cliente , Personas , IDCF , Mesero , Hora , Llevar , des_alternativo , propina) VALUES (:mesa , :cli , :per , :idcf , :idm , :hora , :lle , :des , :pro)";
        ...
        $query->bindParam(':des' , $_POST['deal']);
        $query->bindParam(':pro' , $propina);
        if($query->execute()){
        ...
        if($noland == 0){
            ...
            if($xuery->execute()){
                $fi = $h['IDAC'];
                //header("location: ../../print/orden_cafe.php?final=$fi");
