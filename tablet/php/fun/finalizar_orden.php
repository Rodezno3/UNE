...
if(isset($_GET['id'])){
    ...
    /*- CAMBIAR -*/
    $m = "INSERT INTO Historial_Diario_C (Mesa , Cliente , Mesero , Persona , IDCF , Hora , Fecha , Llevar , des_alternativo , propina) VALUES (:mesa , :clie , :mese , :pers , :idcf , :hora , :fech , :llev , :des , :pro)";
    $n = $conn->prepare($m);
    ...
    $n->bindParam(':des' , $kim['des_alternativo']);
    $n->bindParam(':pro' , $kim['propina']);
    if($n->execute()){
    ...
    if($twi == 1){
        $s = "INSERT INTO Historial_Orden_Diario_C (IDHDC , Producto , Cantidad , Precio) VALUES (:idh , 'descuento' , 1 , :pre)";
        $q = $conn->prepare($s);
        $q->bindParam(':idh' , $misio['IDHDC']);
        $ter = -1 * $_GET['deal'];
        $q->bindParam(':pre' , $ter);
